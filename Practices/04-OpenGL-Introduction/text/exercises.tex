...
For example, you can create a window using Allegro (like we did in the previous sessions) and then use OpenGL functions to draw on it. Instead of Allegro, however, we shall be using the library called ``FreeGLUT'' \emph{(Free GL Utility Toolkit)} to create windows and process events, as it is somewhat more lightweight.

Open the project \verb#1_HelloOpenGL.cbp# in CodeBlocks and study the code. Hopefully you will recognize the familiar structure of initializing the program, defining event handlers, and running the event loop, which will periodically invoke the event handlers and the display routine.
...
Now, however, let us examine a purely three-dimensional example. Open the project file \verb#2_Cube.cbp#. Study the code. Note that it uses perspective projection and the z-buffer algorithm -- something that will be covered in the upcoming lectures. So far just take the provided set up for granted.
...
\begin{exercise}{4}{1}
Finally, open the project \verb#3_Solar.cbp#. Your task here is to implement a simple visualization of the Solar system. As a minimum, you should add four planets (Mercury, Venus, Earth and the Moon), making sure each planet rotates along its own circular orbit at its own pace.
...
Note that GLUT provides you with a bunch of pre-made primitive rendering functions, such as \texttt{glutSolidSphere}. In fact, if you needed to draw a solid-color cube in Exercise 3, you could get away by simply using \texttt{glutSolidCube}.
...
Hint: You do not have to make the character very detailed. Something like a \texttt{glutSolidCylinder} combined with a \texttt{glutSolidSphere} will do. If you want, though, you can try finding a suitable 3D model in the Wavefront .obj format and using it as shown in \verb#4_Object.cbp#.
