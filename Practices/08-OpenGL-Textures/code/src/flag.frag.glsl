...
uniform sampler2D flagImage;  // The only sampler2D uniform is auto-bound to the only texture
...
vec4 blinn(gl_LightSourceParameters light) {
    ...
    attenuation *= pow(max(dot(light.spotDirection, -l), 0), light.spotExponent);
...
void main(void) {
    ...
    c *= texture2D(flagImage, gl_TexCoord[0].xy);
